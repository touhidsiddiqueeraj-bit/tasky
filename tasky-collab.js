...
function _collabAvatarHtml(uid, handle, className) {
    var url = window._vcAvatarCache && window._vcAvatarCache[uid];
    if (url) {
        if (typeof url !== 'string' || !url.startsWith('data:image/')) {
            url = null;
        }
    }
    if (url) return '<div class="member-avatar' + (className ? ' ' + className : '') + '" data-uid="' + uid + '"><img src="' + url + '" alt="@' + handle + '"></div>';
    return '<div class="member-avatar' + (className ? ' ' + className : '') + '" data-uid="' + uid + '" data-handle="' + handle + '">' + handle[0].toUpperCase() + '</div>';
}
function _collabRefreshMemberAvatars() {
    if (!currentGroup || !window._vcAvatarCache) return;
    var pending = [];
    currentGroup.members.forEach(function(m) {
        if (window._vcAvatarCache[m.uid]) return;
        pending.push(
            db.collection('users').doc(m.uid).get().then(function(snap) {
                if (snap.exists && snap.data().avatarDataUrl) {
                    window._vcAvatarCache[m.uid] = snap.data().avatarDataUrl;
                }
            }).catch(function() {})
        );
    });
    if (!pending.length) { _collabApplyAvatarImages(); return; }
    Promise.all(pending).then(_collabApplyAvatarImages);
}
function _collabApplyAvatarImages() {
    document.querySelectorAll('.member-avatar[data-uid]').forEach(function(el) {
        var uid = el.dataset.uid;
        var url = window._vcAvatarCache && window._vcAvatarCache[uid];
        if (url && !el.querySelector('img')) {
            if (typeof url !== 'string' || !url.startsWith('data:image/')) return;
            el.innerHTML = '<img src="' + url + '" alt="@' + (el.dataset.handle || '') + '">';
        }
    });
}
function _collabRefreshAvatar(uid) {
    var url = window._vcAvatarCache && window._vcAvatarCache[uid];
    document.querySelectorAll('.member-avatar[data-uid="' + uid.replace(/[^a-z0-9]/gi,'') + '"],.ro-member-avatar[data-uid="' + uid.replace(/[^a-z0-9]/gi,'') + '"]').forEach(function(el) {
        if (url && typeof url === 'string' && url.startsWith('data:image/')) {
            el.innerHTML = '<img src="' + url.replace(/"/g,'') + '" alt="">';
        } else {
            var h = el.dataset.handle || '';
            el.innerHTML = h ? h[0].toUpperCase() : '?';
        }
    });
}
window._collabRefreshAvatar = _collabRefreshAvatar;
...
let _vcVisitorComments = [];
let _vcVisitorCommentUnsub = null;
...
async function loadActiveGroup() {
    ...
    try {
        ...
        if (serverCode !== localCode) {
            stopGroupListener();
            if (localCode) {
                // Workspace has its own collab code — don't overwrite with server code
                localStorage.setItem(LS_GROUP_KEY, localCode);
                startGroupListener(localCode);
            } else if (serverCode) {
                // Workspace has no code, server has one — check if another workspace owns it
...
// ─── Visitor Comments (Supervisor) ──────────────────────────────────────
function _startVisitorCommentListener(code) {
    if (_vcVisitorCommentUnsub) _vcVisitorCommentUnsub();
    _vcVisitorComments = [];
    _vcVisitorCommentUnsub = db.collection('groups').doc(code).collection('visitor-comments')
        .onSnapshot(function(snap) {
            _vcVisitorComments = [];
            snap.forEach(function(doc) {
                var d = doc.data();
                _vcVisitorComments.push({ id: doc.id, taskNumber: d.taskNumber, taskText: d.taskText, authorName: d.authorName, text: d.text, createdAt: d.createdAt });
            });
            // Update badge in summary bar
            var btn = document.getElementById('vc-feedback-btn');
            if (btn) {
                var count = _vcVisitorComments.length;
                btn.innerHTML = '💬 Visitor Feedback' + (count ? ' <span class="vc-badge">' + count + '</span>' : '');
            }
        });
}

function ensureVisitorFeedbackBtn() {
    var btn = document.getElementById('vc-feedback-btn');
    if (btn) { btn.style.display = ''; return; }
    var preview = document.getElementById('collab-share-link-preview');
    if (!preview) return;
    var count = _vcVisitorComments ? _vcVisitorComments.length : 0;
    var wrap = document.createElement('div');
    wrap.id = 'vc-feedback-wrapper';
    wrap.style.cssText = 'margin-top:10px;';
    wrap.innerHTML = '<button id="vc-feedback-btn" style="background:rgba(139,92,246,0.12);border:1px solid rgba(139,92,246,0.2);border-radius:8px;padding:6px 14px;font-size:12px;color:#a78bfa;cursor:pointer;">'
        + '💬 Visitor Feedback' + (count ? ' <span class="vc-badge">' + count + '</span>' : '')
        + '</button>';
    preview.after(wrap);
    document.getElementById('vc-feedback-btn').addEventListener('click', _openVisitorCommentsModal);
}

function _openVisitorCommentsModal() {
    var existing = document.getElementById('vc-modal');
    if (existing) existing.remove();
    var existingOv = document.getElementById('vc-modal-overlay');
    ...
    var html = '<div class="vc-modal-header"><span>💬 Visitor Feedback</span><button id="vc-modal-close">✕</button></div>';
    html += '<div class="vc-modal-body">';

    if (!_vcVisitorComments.length) {
        html += '<div style="text-align:center;padding:30px;color:rgba(255,255,255,0.3);">No visitor comments yet.<br>Share your live board link to receive feedback.</div>';
    } else {
        _vcVisitorComments.forEach(function(c) {
            var time = c.createdAt && c.createdAt.toDate ? c.createdAt.toDate().toLocaleTimeString('en-US', { hour:'2-digit', minute:'2-digit' }) : '';
            html += '<div class="vc-comment-card">'
                + '<div class="vc-comment-head">'
                + '<span class="vc-comment-author">' + escHtml(c.authorName) + '</span>'
                + (time ? '<span class="vc-comment-time">' + time + '</span>' : '')
                + '<span class="vc-comment-task"># ' + c.taskNumber + '</span>'
                + '</div>'
                + '<div class="vc-comment-text">' + escHtml(c.text) + '</div>'
                + '<div style="margin-top:6px;"></div>' // reply placeholder
                + '</div>';
        });
    }
    ...
    document.getElementById('vc-modal-close').addEventListener('click', function() {
        modal.remove();
        overlay.remove();
    });
}
...
function startGroupListener(code) {
    ...
    groupListener = db.collection('groups').doc(code).onSnapshot({ includeMetadataChanges: false }, async snap => {
        ...
        _syncCollabState();
        renderGroupUI();
        _drainPendingInlineComments();

        if (isSupervisor) {
            stopTasksListener();
            teamTasksCache = {};
            startTasksListener(code);
            _startVisitorCommentListener(code);
        } else {
            stopTasksListener();
            _startVisitorCommentListener(code);
        }
        ...
        .collection('tasks').onSnapshot(snap => {
            ...
            if (list && currentGroup && isSupervisor && !teamPanelMember) {
                ...
                currentGroup.members.forEach(m => {
                    ...
                    card.innerHTML = `
                        ${_collabAvatarHtml(m.uid, m.handle, isSup ? 'supervisor' : '')}
...
function stopGroupListener() {
    ...
    stopTasksListener();
    if (_vcVisitorCommentUnsub) { _vcVisitorCommentUnsub(); _vcVisitorCommentUnsub = null; _vcVisitorComments = []; }
}
...
function renderGroupUI() {
    ...
    if (wsCode) {
        ...
        if (mc) mc.remove();
        // Voice hook handles vcLeave in its else branch
    ...
    currentGroup.members.forEach(m => {
        ...
        row.innerHTML = `
            <div class="tst-cell tst-member-col">
                ${_collabAvatarHtml(m.uid, m.handle, 'tiny' + (isSup ? ' supervisor' : ''))}
...
function renderMemberDetail(list) {
    ...
    header.innerHTML = `
        ...
        <div class="member-detail-name">
            <div class="member-avatar small" data-uid="${member.uid}" data-handle="${member.handle}">${_mbAvatarInner(member.uid,member.handle)}</div>
            @${member.handle}
        </div>
    `;
...
async function renderCollabSummary() {
    ...
    if (!bar) {
        ...
        var _container = document.querySelector('.container') || document.querySelector('.board') || document.body;
    ...
    currentGroup.members.forEach(m => {
        ...
        row.innerHTML = `
            <div class="summary-member">
                ${_collabAvatarHtml(m.uid, m.handle, 'tiny' + (m.uid === currentGroup.supervisorUid ? ' supervisor' : ''))}
                <span>@${m.handle}</span>
            </div>
...
function buildCollabModal() {
    ...
    <div class="tg-modal" style="width:min(520px,96vw);">
        ...
        <div class="tg-body" style="padding:0;">
            <!-- Sign-in prompt pane (separate from handle pane so DOM isn't destroyed) -->
            <div id="collab-pane-signin" class="collab-pane" style="display:none;padding:28px;">
                <div style="text-align:center;padding:16px 0;">
                    ...
                    <button class="tg-save-btn" onclick="signInWithGoogle();closeCollabModal();">Sign in with Google</button>
                </div>
            </div>
            <!-- Handle setup pane -->
...
async function showCollabModalPane(mode) {
    ...
    if (!isRealUser && mode !== 'info' && mode !== 'change') {
        const pane = document.getElementById('collab-pane-signin');
...
(function initReadOnlyMode() {
    ...
    // Validate viewCode: only allow alphanumeric codes (group codes are uppercase letters)
    if (!/^[A-Za-z0-9]{3,20}$/.test(viewCode)) {
        console.warn('[tasky] Invalid read-only board code');
        return;
    }

    // Throttle: prevent rapid re-mounts / brute-force
    var _lastViewAttempt = 0;
    var _viewMinInterval = 1000;
    var now = Date.now();
    if (now - _lastViewAttempt < _viewMinInterval && _lastViewAttempt !== 0) return;
    _lastViewAttempt = now;
...
(function initInviteMode() {
    ...
    // Persist the code through auth flow (user may need to sign in first)
    sessionStorage.setItem('tasky_pending_invite', joinCode);
...
async function _mountReadOnlyBoard(code) {
    ...
    shell.innerHTML = `
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ro-member-avatar img { width:100%; height:100%; border-radius:50%; object-fit:cover; }
        .ro-section-label {
        ...
        .ro-task-wrapper { margin-bottom: 4px; }
        .ro-comment-toggle { background:none; border:none; cursor:pointer; font-size:11px; color:rgba(255,255,255,0.35); padding:4px 0 0 2px; display:flex; align-items:center; gap:4px; }
        .ro-comment-toggle:hover { color:#a78bfa; }
        .ro-comment-section { margin-top:6px; }
        .ro-reply-link { font-size:11px; color:rgba(255,255,255,0.3); cursor:pointer; background:none; border:none; padding:0; }
        .ro-reply-link:hover { color:#a78bfa; }
        .ro-cf-reply-row { display:flex; gap:4px; margin-top:4px; }
        </style>
        <div class="ro-topbar">
            <div class="ro-brand">
...
match /groups/{code}/tasks/{uid} { allow read: if true; }</code>
            ...
            <div class="ro-members">
                ${members.map(m => `
                    <div class="ro-member-chip">
                        <div class="ro-member-avatar" data-uid="${m.uid}" data-handle="${m.handle}">${_mbAvatarInner(m.uid,m.handle)}</div>
                        @${escHtml(m.handle)}
    ...
    function _roRefreshCommentUI(num) {
        var toggle = document.querySelector('.ro-comment-toggle[data-task-number="' + num + '"]');
        if (toggle) {
            var cc = (_roCommentCache[num] || []).length;
            var badge = toggle.querySelector('.ro-cc-num');
            if (badge) badge.textContent = cc || 'Comment';
        }
        var section = document.getElementById('ro-cs-' + num);
        if (section && section.style.display !== 'none') {
            var btn = document.querySelector('.ro-comment-toggle[data-task-number="' + num + '"]');
            var txt = btn ? btn.dataset.taskText : '';
            _roToggleComments(num, txt);
        }
    }

    function _roStartCommentListener(code) {
        if (_roCommentUnsub) _roCommentUnsub();
        _roCommentUnsub = db.collection('groups').doc(code).collection('visitor-comments')
            .onSnapshot(function(snap) {
                snap.docChanges().forEach(function(change) {
                    var changeNum = change.doc.data().taskNumber;
                    if (changeNum == null) return;
                    if (change.type === 'removed') {
                        var c = _roCommentCache[changeNum];
                        if (c) _roCommentCache[changeNum] = c.filter(function(x) { return x.id !== change.doc.id; });
                    } else {
                        var d = change.doc.data();
                        let num = d.taskNumber;
                        if (!_roCommentCache[num]) _roCommentCache[num] = [];
                        var idx = _roCommentCache[num].findIndex(function(x) { return x.id === change.doc.id; });
                        var entry = { id: change.doc.id, text: d.text, authorName: d.authorName, authorEmail: d.authorEmail, createdAt: d.createdAt, replies: d._replies || [] };
                        if (idx >= 0) _roCommentCache[num][idx] = entry;
                        else _roCommentCache[num].push(entry);
                        if (!d._replyListenerStarted) {
                            db.collection('groups').doc(code).collection('visitor-comments').doc(change.doc.id).collection('replies')
                                .onSnapshot(function(rsnap) {
                                    var repls = [];
                                    rsnap.forEach(function(r) { repls.push({ id: r.id, text: r.data().text, authorName: r.data().authorName, createdAt: r.data().createdAt }); });
                                    var arr = _roCommentCache[num];
                                    if (arr) {
                                        var e = arr.find(function(x) { return x.id === change.doc.id; });
                                        if (e) { e._replies = repls; e._replyListenerStarted = true; }
                                    }
                                    _roRefreshCommentUI(num);
                                });
                        }
                    }
                    _roRefreshCommentUI(changeNum);
                });
            });
    }

    function _roToggleComments(taskNumber, taskText) {
        var section = document.getElementById('ro-cs-' + taskNumber);
        if (!section) return;
        if (section.style.display !== 'none') { section.style.display = 'none'; return; }
        section.style.display = 'block';
        var list = document.getElementById('ro-cl-' + taskNumber);
        if (!list) return;
        var md = typeof window._taskyRenderMarkdown === 'function';
        function _rmd(s) { return md ? window._taskyRenderMarkdown(s) : escHtml(s); }
        var html = '<div class="ic-strip">';
        var comments = _roCommentCache[taskNumber] || [];
        comments.forEach(function(c) {
            var time = c.createdAt && c.createdAt.toDate ? _roFmtTime(c.createdAt.toDate()) : '';
            html += '<div class="ic-entry">'
                + '<span class="ic-author">' + escHtml(c.authorName) + '</span>'
                + '<span class="ic-text">' + _rmd(c.text) + '</span>'
                + (time ? '<span class="ic-ts">' + time + '</span>' : '')
                + '</div>';
            (c._replies || []).forEach(function(r) {
                var rt = r.createdAt && r.createdAt.toDate ? _roFmtTime(r.createdAt.toDate()) : '';
                html += '<div class="ic-entry" style="margin-left:16px;padding-left:10px;border-left:2px solid rgba(139,92,246,0.2);">'
                    + '<span class="ic-author" style="color:#6ee7b7;">' + escHtml(r.authorName) + '</span>'
                    + '<span class="ic-text">' + _rmd(r.text) + '</span>'
                    + (rt ? '<span class="ic-ts">' + rt + '</span>' : '')
                    + '</div>';
            });
            html += '<div style="margin:2px 0 4px 0;"><button class="ro-reply-link" data-comment-id="' + c.id + '">↩ Reply</button></div>'
                + '<div class="ro-cf-reply-row" id="ro-reply-row-' + c.id + '" style="display:none;">'
                + '<input type="text" name="vc-reply" id="ro-reply-input-' + c.id + '" placeholder="Write a reply…" class="tcp-input" style="font-size:12px;padding:6px 9px;">'
                + '<button class="ro-cf-reply-send" data-comment-id="' + c.id + '" style="background:linear-gradient(135deg,#8B5CF6,#6d28d9);border:none;border-radius:8px;padding:6px 10px;font-size:12px;color:#fff;cursor:pointer;">↑</button>'
                + '</div>';
        });
        html += '</div>';
        html += '<div class="ro-comment-form" data-task-number="' + taskNumber + '" data-task-text="' + taskText + '" style="margin-top:8px;border-top:1px solid rgba(255,255,255,0.06);padding-top:8px;">'
            + '<input class="tcp-input ro-cf-name" name="vc-name" id="vc-name-' + taskNumber + '" placeholder="Your name *" required style="font-size:12px;padding:7px 10px;margin-bottom:4px;">'
            + '<input class="tcp-input ro-cf-email" name="vc-email" id="vc-email-' + taskNumber + '" placeholder="Your email (optional)" style="font-size:12px;padding:7px 10px;margin-bottom:4px;">'
            + '<textarea class="tcp-input ro-cf-text" name="vc-text" id="vc-text-' + taskNumber + '" placeholder="Write a comment…" rows="2" style="font-size:12px;padding:7px 10px;margin-bottom:4px;resize:vertical;min-height:40px;"></textarea>'
            + '<button class="tcp-send ro-cf-submit" data-task-number="' + taskNumber + '" data-task-text="' + taskText + '" style="font-size:12px;padding:7px 14px;border-radius:8px;">Post Comment</button>'
            + '</div>';
        list.innerHTML = html;
    }

    function _roHandleSubmit(subBtn) {
        var form = subBtn.closest('.ro-comment-form');
        if (!form) return;
        var num = form.dataset.taskNumber;
        var ttext = form.dataset.taskText;
        var nameInput = form.querySelector('.ro-cf-name');
        var emailInput = form.querySelector('.ro-cf-email');
        var textInput = form.querySelector('.ro-cf-text');
        var name = nameInput ? nameInput.value.trim() : '';
        var email = emailInput ? emailInput.value.trim() : '';
        var text = textInput ? textInput.value.trim() : '';
        if (!name || !text) { alert('Please enter your name and a comment.'); return; }
        _roSubmitComment(code, num, ttext, name, email, text);
        if (textInput) textInput.value = '';
    }

    function _roHandleReply(repBtn) {
        var cid = repBtn.dataset.commentId;
        var input = document.getElementById('ro-reply-input-' + cid);
        if (input && input.value.trim()) {
            _roSubmitReply(code, cid, input.value.trim());
            input.value = '';
            var row = document.getElementById('ro-reply-row-' + cid);
            if (row) row.style.display = 'none';
        }
    }

    function _roSubmitComment(code, taskNumber, taskText, authorName, authorEmail, text) {
        db.collection('groups').doc(code).collection('visitor-comments').add({
            ...
            text: text,
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        }).catch(function(err) { console.error('Comment failed', err); });
    }

    function _roSubmitReply(code, commentId, text) {
        db.collection('groups').doc(code).collection('visitor-comments').doc(commentId).collection('replies').add({
            authorName: 'Supervisor',
            text: text,
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        }).catch(function(err) { console.error('Reply failed', err); });
    }

    function _roFmtTime(date) {
    ...
    function _renderROBoard(group, cache) {
        ...
        function taskHTML(t) {
            ...
            var cc = (_roCommentCache[t.number] || []).length;
            return `
                <div class="ro-task-wrapper">
                    <div class="ro-task">
                    ...
                    <button class="ro-comment-toggle" data-task-number="${t.number}" data-task-text="${escHtml(t.text)}">
                        💬 <span class="ro-cc-num">${cc || 'Comment'}</span>
                    </button>
                    <div class="ro-comment-section" id="ro-cs-${t.number}" style="display:none;">
                        <div class="ro-comments-list" id="ro-cl-${t.number}"></div>
                    </div>
                </div>`;
        }
        ...
        if (!board._roListenerAttached) {
            board._roListenerAttached = true;
            board.addEventListener('click', function _roBoardClick(e) {
            var btn = e.target.closest('.ro-comment-toggle');
            if (btn) { _roToggleComments(btn.dataset.taskNumber, btn.dataset.taskText); return; }
            var sub = e.target.closest('.ro-cf-submit');
            if (sub) { _roHandleSubmit(sub); return; }
            var rep = e.target.closest('.ro-cf-reply-send');
            if (rep) { _roHandleReply(rep); return; }
            var rl = e.target.closest('.ro-reply-link');
            if (rl) {
                var cid = rl.dataset.commentId;
                var row = document.getElementById('ro-reply-row-' + cid);
                if (row) row.style.display = row.style.display === 'none' ? 'flex' : 'none';
                if (row && row.style.display === 'flex') {
                    var inp = document.getElementById('ro-reply-input-' + cid);
                    if (inp) inp.focus();
                }
            }
        });
...
async function _handleAuthChange() {
    ...
    if (user && !user.isAnonymous) {
                    ...
                    avatarListener = window.db.collection('users').doc(user.uid)
                        .onSnapshot(snap => {
                            ...
                            if (url) {
                                ...
                                // Refresh collab member list & read-only board avatar
                                _collabRefreshAvatar(user.uid);
                                // Refresh message board composer if open
                                _mbRefreshComposerAvatar();
        ...
        // Re-trigger invite join if user signed in after clicking ?join=
        const pendingInvite = sessionStorage.getItem('tasky_pending_invite');
        if (pendingInvite) {
            sessionStorage.removeItem('tasky_pending_invite');
            openCollabModal('join');
            setTimeout(() => {
                const input = document.getElementById('collab-join-code-input');
                if (input) {
                    input.value = pendingInvite.toUpperCase().trim();
                    input.dispatchEvent(new Event('input'));
                }
            }, 100);
        }
...
window.__onWorkspaceSwitch = function(newId, oldId) {
    ...
    if (ws && ws.collabCode) {
        ...
        teamPanelMember = null;
        teamTasksCache = {};
        // Remove stale collab UI immediately — don't let renderGroupUI run with
        // stale currentGroup before the async Firestore listener settles.
        var staleCol = document.getElementById('collab-team-column');
        if (staleCol) staleCol.remove();
        var _board = document.querySelector('.board');
        if (_board) _board.classList.remove('board-4col');
        // Start listener — onSnapshot will set currentGroup and call renderGroupUI
        // with fresh data once Firestore responds.
...
// Persisted to localStorage per-workspace so comments survive page refresh.
var _soloActivity = {};   // { [taskId]: [ entry, … ] }
function _saveSoloActivity() {
    try { localStorage.setItem('_soloActivity_' + activeWorkspaceId, JSON.stringify(_soloActivity)); } catch(_) {}
}
function _loadSoloActivity() {
    try {
        var saved = localStorage.getItem('_soloActivity_' + activeWorkspaceId);
        if (saved) { _soloActivity = JSON.parse(saved); return; }
    } catch(_) {}
    _soloActivity = {};
}
_loadSoloActivity();

// Queue of task IDs whose inline comments couldn't load because currentGroup
// wasn't available when the task card was created. Drained after group loads.
var _pendingInlineComments = [];
...
// ─── Load inline comments from Firestore for a single task ──────────────
function _loadInlineComments(taskId) {
    if (!currentGroup || !db) return;
    var key = 'comments_' + String(taskId).replace(/[^a-z0-9]/gi,'_');
    var memberUids = (currentGroup.members || []).map(function(m) { return m.uid; });
    if (memberUids.length === 0) return;
    Promise.all(
        memberUids.map(function(uid) {
            return db.collection('groups').doc(currentGroup.code)
                .collection('tasks').doc(uid).get();
        })
    ).then(function(snaps) {
        ...
        _renderInlineComments(taskId, allEntries);
    }).catch(function() {});
}
function _drainPendingInlineComments() {
    if (!currentGroup || !db || _pendingInlineComments.length === 0) return;
    var ids = _pendingInlineComments.slice();
    _pendingInlineComments = [];
    ids.forEach(function(id) { _loadInlineComments(id); });
}
...
async function openComments(taskId, taskText, column, ownerUid) {
    ...
    input.addEventListener('keydown', e => {
        e.stopPropagation();
        if (e.key === 'Enter') {
            if (typeof _mentionDropdown !== 'undefined' && _mentionDropdown && _mentionDropdown.classList.contains('visible')) {
                var _focused = _mentionDropdown.querySelector('.mention-item.focused');
                if (_focused) { return; }
            }
...
createTaskCard = function(task, column) {
    ...
    if (currentGroup) {
        _loadInlineComments(task.id);
    } else {
        // Solo mode — use in-memory activity store
        var soloEntries = _soloActivity[task.id] || [];
        if (soloEntries.length > 0) {
            _renderInlineComments(task.id, soloEntries);
        } else if (typeof currentGroup === 'undefined' || currentGroup === null) {
            // Group not loaded yet — defer until _handleAuthChange sets currentGroup
            if (_pendingInlineComments.indexOf(task.id) === -1) _pendingInlineComments.push(task.id);
        }
    }
    return card;
};
...
// ─── Message board avatar helper ───────────────────────────────────────────
function _mbAvatarInner(uid, handle) {
    var url = uid && window._vcAvatarCache && window._vcAvatarCache[uid];
    if (url && typeof url === 'string' && url.startsWith('data:image/'))
        return '<img src="' + url.replace(/"/g,'') + '" alt="@' + (handle || '') + '">';
    return (handle || '?')[0].toUpperCase();
}
function _mbRefreshComposerAvatar() {
    var el = document.querySelector('.mb-composer .mb-avatar-sm');
    if (!el || !currentUser) return;
    el.innerHTML = _mbAvatarInner(currentUser.uid, _mbMe());
}
window._mbRefreshComposerAvatar = _mbRefreshComposerAvatar;
...
function _mbBuildPanel() {
    ...
    panel.innerHTML = `
        ...
        <div class="mb-composer">
            <div class="mb-composer-inner">
                <div class="mb-avatar-sm">${_mbAvatarInner(currentUser?currentUser.uid:null,_mbMe())}</div>
    ...
    ta.addEventListener('input', function() {
        ...
        if (typeof _getGroupMembers !== 'function' || typeof _showMentionDD !== 'function') return;
        var val = ta.value, caret = ta.selectionStart;
        var atPos = -1;
        ...
        if (atPos !== -1 && _getGroupMembers().length > 0) {
            ...
            _showMentionDD(ta, val.slice(atPos + 1, caret));
        } else if (typeof _hideMentionDD === 'function') {
            _hideMentionDD();
        }
    });
    ta.addEventListener('keydown', function(e) {
        if (_mentionDropdown && _mentionDropdown.classList.contains('visible')) {
            var _items = _mentionDropdown.querySelectorAll('.mention-item');
            if (e.key === 'ArrowDown')  { e.preventDefault(); if (typeof _updateMentionFocus === 'function') _updateMentionFocus((_mentionFocusIdx + 1) % _items.length); return; }
            if (e.key === 'ArrowUp')    { e.preventDefault(); if (typeof _updateMentionFocus === 'function') _updateMentionFocus((_mentionFocusIdx - 1 + _items.length) % _items.length); return; }
            if (e.key === 'Enter' || e.key === 'Tab') {
                var _focused = _mentionDropdown.querySelector('.mention-item.focused');
                if (_focused) { e.preventDefault(); e.stopImmediatePropagation(); if (typeof _insertMention === 'function') _insertMention(_focused.dataset.handle); return; }
            }
            if (e.key === 'Escape') { if (typeof _hideMentionDD === 'function') _hideMentionDD(); return; }
        }
        if (e.key==='Enter'&&!e.shiftKey){e.preventDefault();_mbSend();}
    });
    ta.addEventListener('blur', function() { setTimeout(function() { if (typeof _hideMentionDD === 'function') _hideMentionDD(); }, 150); });
...
// ─── File attach ──────────────────────────────────────────────────────────
function _mbSetPendingFile(name, type, dataUrl) {
    _mbPendingFile = { name: name, type: type, dataUrl: dataUrl };
    const pv = document.getElementById('mb-attach-preview');
    if (!pv) return;
    pv.style.display = 'flex';
    const isImg = type.startsWith('image/');
    pv.innerHTML = isImg
        ? `<img src="${dataUrl}" class="mb-attach-thumb" alt="${escHtml(name)}"><span class="mb-attach-name">${escHtml(name)}</span><button class="mb-attach-remove">✕</button>`
        : `<span class="mb-attach-icon">📎</span><span class="mb-attach-name">${escHtml(name)}</span><button class="mb-attach-remove">✕</button>`;
    pv.querySelector('.mb-attach-remove').addEventListener('click',()=>{
        _mbPendingFile=null; pv.innerHTML=''; pv.style.display='none';
    });
}
function _mbHandleFile(input) {
    ...
    reader.onload=e=>{
        // If image and photo editor available, open editor
        if (file.type.startsWith('image/') && typeof window._photoOpenEditor === 'function') {
            window._photoOpenEditor(e.target.result, file.name,
                function(edited) {
                    // Cancel — just clear
                    input.value = '';
                },
                function(edited) {
                    // Send — attach then send immediately
                    if (edited) _mbSetPendingFile(file.name, file.type, edited);
                    input.value = '';
                    _mbSend();
                }
            );
        ...
        _mbSetPendingFile(file.name, file.type, e.target.result);
    };
    reader.readAsDataURL(file);
}

// ─── Image lightbox (fullscreen overlay) ──────────────────────────────────
function _mbShowImageLightbox(dataUrl, name) {
    var overlay = document.createElement('div');
    ...
    img.src = dataUrl;
    img.alt = name || '';
    img.style.cssText = 'max-width:92vw;max-height:92vh;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,0.6);object-fit:contain;';
    overlay.appendChild(img);
    var cap = document.createElement('div');
    cap.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);color:rgba(255,255,255,0.5);font-size:13px;font-family:sans-serif;text-align:center;pointer-events:none;';
...
function _mbRenderFeed() {
    ...
    _mbMessages.forEach(msg=>{
        ...
        if(msg.attachment){
            ...
            if(typeof a.dataUrl !== 'string') a.dataUrl = '';
            if(typeof a.type !== 'string') a.type = '';
            var isImage = a.type.startsWith('image/') && a.dataUrl.startsWith('data:image/');
            var isSafeDataUrl = a.dataUrl.startsWith('data:') && !a.dataUrl.startsWith('data:text/html') && !a.dataUrl.startsWith('data:application/x-javascript');
            var _attachDataUrl = escHtml(a.dataUrl);
            var _attachName = escHtml(a.name);
            attachHtml=isImage && isSafeDataUrl
                ? `<img src="${_attachDataUrl}" class="mb-attach-img" alt="${_attachName}" style="cursor:zoom-in">`
                : isSafeDataUrl
                    ? `<a class="mb-file-link" href="${_attachDataUrl}" download="${_attachName}">📎 ${_attachName}</a>`
                    : `<a class="mb-file-link" href="#">📎 ${_attachName} (unsupported file)</a>`;
        }
        ...
        card.innerHTML=`
            <div class="mb-msg-meta">
                <span class="mb-avatar-sm ${msg.authorUid===(currentGroup&&currentGroup.supervisorUid)?'mb-avatar--sup':''}">${_mbAvatarInner(msg.authorUid,msg.authorHandle)}</span>
            ...
            ${msg.text?`<div class="rt-body mb-msg-text">${window._taskyRenderMarkdown(msg.text)}</div>`:''}
            ${attachHtml}
            ...
            <div class="mb-thread" id="mb-thread-${msg.id}" style="display:${_mbReplyOpenId===msg.id?'block':'none'};">
                ...
                <div class="mb-reply-composer">
                    <div class="mb-avatar-xs">${_mbAvatarInner(currentUser?currentUser.uid:null,me)}</div>
        ...
        // Wire image lightbox for attachment images
        var _attachImg = card.querySelector('.mb-attach-img');
        if (_attachImg) {
            _attachImg.addEventListener('click', function() {
                _mbShowImageLightbox(this.src, this.alt);
            });
        }
        ...
        if(_mbReplyOpenId===msg.id){
            ...
            if(ri) {
                ri.addEventListener('input', function() {
                    if (typeof _getGroupMembers !== 'function' || typeof _showMentionDD !== 'function') return;
                    var val = ri.value, caret = ri.selectionStart;
                    var atPos = -1;
                    for (var i = caret - 1; i >= 0; i--) {
                        if (val[i] === '@') { atPos = i; break; }
                        if (val[i] === ' ') break;
                    ...
                    if (atPos !== -1 && _getGroupMembers().length > 0) {
                        _mentionStartIdx = atPos;
                        _showMentionDD(ri, val.slice(atPos + 1, caret));
                    } else if (typeof _hideMentionDD === 'function') {
                        _hideMentionDD();
                    }
                });
                ...
                ri.addEventListener('blur', function() { setTimeout(function() { if (typeof _hideMentionDD === 'function') _hideMentionDD(); }, 150); });
            }
...
function _mbRenderReplies(msg) {
    ...
    (msg.replies||[]).forEach(r=>{
        ...
        div.innerHTML=`
            <div class="mb-msg-meta">
                <span class="mb-avatar-xs">${_mbAvatarInner(r.authorUid,r.authorHandle)}</span>
            ...
            <div class="rt-body mb-msg-text mb-reply-text">${window._taskyRenderMarkdown(r.text||'')}</div>
