...
.mention-avatar img { width:100%; height:100%; border-radius:50%; object-fit:cover; }
...
function _showMentionDD(input, query) {
    ...
    filtered.forEach(function(h, i) {
        ...
        item.dataset.handle = h;
        item.dataset.idx = i;

        var avatar = document.createElement('div');
        avatar.className = 'mention-avatar';
        var member = currentGroup && currentGroup.members.find(function(m) { return m.handle === h; });
        var mUid = member && member.uid;
        var mUrl = mUid && window._vcAvatarCache && window._vcAvatarCache[mUid];
        if (mUrl && typeof mUrl === 'string' && mUrl.startsWith('data:image/')) {
            var mImg = document.createElement('img');
            mImg.src = mUrl;
            mImg.style.cssText = 'width:100%;height:100%;border-radius:50%;object-fit:cover;';
            avatar.appendChild(mImg);
        } else {
            avatar.textContent = h[0].toUpperCase();
        }

        var label = document.createElement('span');
        label.className = 'mention-handle';
        label.textContent = '@' + h;

        item.appendChild(avatar);
        item.appendChild(label);

        if (typeof currentHandle !== 'undefined' && h === currentHandle) {
            var you = document.createElement('span');
            you.className = 'mention-you';
            you.textContent = 'you';
...
// Sync mention notif button state on settings open (augment, don't replace)
var _origOpenSettings = window.openSettings;
window.openSettings = function() {
    if (typeof _origOpenSettings === 'function') _origOpenSettings();
    setTimeout(function() {
        var btn = document.getElementById('st-mention-notif-btn');
        if (btn) {
